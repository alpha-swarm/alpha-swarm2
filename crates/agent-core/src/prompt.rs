...
const TOOL_FORMAT: &str = "\nOUTPUT FORMAT:\nYou can call tools and edit files. Output ONLY <<< blocks.\n\n<<<TOOL tool_name\n{\"param\": \"value\"}\n>>>\n\n<<<EDIT {actual_file_from_task}\n--- OLD\nexact existing lines\n--- NEW\nreplacement lines\n>>>\n\n<<<CREATE {actual_file_from_task}\nfile contents\n>>>\n\n<<<DONE\nwhat was done\n>>>\n\nWORKFLOW:\n1. Read files with read_file tool\n2. Make edits with <<<EDIT>>> blocks\n3. Run run_build to check compilation\n4. If build fails, read the error and fix with another <<<EDIT>>>\n5. Run run_test to verify\n6. If tests fail, fix and retest\n7. <<<DONE>>> when everything passes\n\nRULES:\n- Use ONLY file paths from the FILES section or TASK description\n- Never use example/placeholder paths\n- Start with <<< immediately, no explanation\n- After editing code files, ALWAYS run run_build to check compilation";
