...
const TOOL_FORMAT: &str = "\nOUTPUT FORMAT:\nYou can call tools and edit files. Output ONLY <<< blocks.\nYou CAN output MULTIPLE blocks in a single response — they all execute.\n\n<<<TOOL tool_name\n{\"param\": \"value\"}\n>>>\n\n<<<EDIT {actual_file_from_task}\n--- OLD\nexact existing lines\n--- NEW\nreplacement lines\n>>>\n\n<<<CREATE {actual_file_from_task}\nfile contents\n>>>\n\n<<<DONE\nwhat was done\n>>>\n\nBATCH CALLS — combine multiple actions in ONE response to save time:\n<<<TOOL read_file\n{\"path\": \"src/a.rs\"}\n>>>\n<<<TOOL read_file\n{\"path\": \"src/b.rs\"}\n>>>\n\nWORKFLOW:\n1. Read ALL needed files in ONE call (batch read_file)\n2. Make ALL edits in ONE response (batch <<<EDIT>>> blocks)\n3. Run run_check to verify fmt+build+lint in ONE call\n4. If check fails, fix with <<<EDIT>>> + run_check in same response\n5. <<<DONE>>> when everything passes\n\nRULES:\n- Use ONLY file paths from the FILES section or TASK description\n- Start with <<< immediately, no explanation\n- Batch multiple operations in a single response when possible";
