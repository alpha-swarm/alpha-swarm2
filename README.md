...
graph TD;
    ...
    subgraph csatapaci["csatapaci (M2 Max 96GB)"]
        Ollama["Ollama 72B/33B/7B"]
        Daemon2["Daemon"]
        NATS2["NATS"]
    end

    subgraph malna["malna (RPi 5 8GB)"]
        NATS3["NATS quorum"]
        WASITools["WASI tools"]
    end

    NATS1 <-->|Tailscale mesh| NATS2
    NATS1 <-->|Tailscale mesh| NATS3
    NATS2 <-->|Tailscale mesh| NATS3
```
